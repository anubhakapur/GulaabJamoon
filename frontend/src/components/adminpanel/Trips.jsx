...
const Trips = () => {
  ...
  const fetchExperiences = async () => {
    try {
      const response = await axios.get(`${BASE_URL}/api/all-experiences`);
      ...
      if (response.data.success) {
        ...
        const enrichedExperiences = await Promise.all(
          response.data.data.map(async (exp) => {
            const bookingsResponse = await axios.get(
              `${BASE_URL}/api/bookings-count/${exp._id}`
            );
  ...
  const handleAction = async (action, id) => {
    try {
      if (action === "edit") {
        ...
        const response = await axios.put(`${BASE_URL}/api/change-status`, {
          id: id,
          status: newStatus,
        });
            ...
            (
              <tr key={exp._id}>
                ...
                <td className="px-6 py-4 whitespace-nowrap">
                  ...
                  <FaLink
                    onClick={() => navigate(`/experiences/${exp.url}`)}
                    className="inline cursor-pointer text-gray-600 hover:text-gray-900 text-xl"
                  />
                </td>
                <td className="px-6 py-4 whitespace-nowrap">
                  <span
                ...
                <td className="px-6 py-4 whitespace-nowrap text-sm font-medium">
                  {editingId === exp.id ? (
                    <button
                      onClick={() => handleAction("save", exp.id)}
                    ...
                    <button
                      onClick={() => handleAction("edit", exp.id)}
                  ...
                  {exp.status === "Live" ? (
                    <button
                      onClick={() => handleAction("hold", exp.id)}
                    ...
                    <button
                      onClick={() => handleAction("goLive", exp.id)}
